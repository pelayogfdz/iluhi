...
export async function generateCotizacionPdf(factura, empresa, cliente, facturapiClient) {
  ...
  fInvoice.items.forEach(item => {
     itemsTable.push([
       ...
       `$${item.product.price.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
       `$${(item.quantity * item.product.price).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
     ]);
  });

  const totals = [
      { text: `Subtotal: $${factura.subTotal.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, margin: [0, 5, 0, 5], bold: true },
      { text: `Total Impuestos: $${factura.totalImpuestosTrasladados.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, margin: [0, 0, 0, 5] },
      { text: `TOTAL: $${factura.total.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${factura.moneda || 'MXN'}`, bold: true, fontSize: 14, color: empresa.colorPrimario || '#0054a6' }
  ];
