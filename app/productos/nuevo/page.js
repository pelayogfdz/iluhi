...
async function createProducto(formData) {
  ...
  const claveProdServ = rawProd.toString().split(' - ')[0].trim()
  
  const rawUnidad = formData.get('claveUnidad') || ''
  const claveUnidad = rawUnidad.toString().split(' - ')[0].trim()
      ...
      <div className="glass-panel" style={{ maxWidth: '800px' }}>
        <form action={createProducto} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="form-grid-3">
              ...
              <div className="form-group">
                <label htmlFor="tasaOCuota">Tasa o Cuota (IVA / IEPS)</label>
                <select id="tasaOCuota" name="tasaOCuota" className="form-control" required style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                  ...
                  <option value="0.265">26.5% (0.265) - IEPS</option>
                </select>
              </div>
          </div>
